...
* CS475
* Program #: 6
* Program Name: OpenCL Array Multiplication, Multiply-Add, and Multiply-Reduce (Part 1)
******************************************************************************/
#include <stdio.h>
#include <math.h>
#include <string.h>
...
// Conditional Experiment type
#if EXP_TYPE == 1
const char *		CL_FILE_NAME = { "multiply.cl" };	// Use mulitply kernal
#endif

#if EXP_TYPE == 2
const char *		CL_FILE_NAME = { "multiplyAdd.cl" };	// Use multiply add kernal
#endif
...
void printResults(double GigaMultsPerSecond);
...
main( int argc, char *argv[ ] )
{
	...
	float *hD = new float[ NUM_ELEMENTS ];		// Additional memory buffer allocated to accomodate multiplyAdd
	...
	for( int i = 0; i < NUM_ELEMENTS; i++ )
	{
		...
		hC[i] = 1.; 			// Holds addition values for multiply add kernal (not used by multiply kernal)
	}
	...
	cl_mem dD = clCreateBuffer( context, CL_MEM_WRITE_ONLY, dataSize, NULL, &status );	// Extra buffer (holds multiplyAdd results)
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (3)\n" );
	...
	printResults(GigaMultsPerSecond);
...
void printResults(double GigaMultsPerSecond)
{
	...
	std::ofstream p1Results;
	p1Results.open("p1Results.txt", std::ios::out | std::ios::app);

    

    // Print Experiment Type and Headings (if applicable)
    if(NUM_ELEMENTS == 1024 && LOCAL_SIZE == 1)
    {
        if(EXP_TYPE == 1)
        {
            std::cout << "\n-----------------------------------------------------\n";
        ...
        else
        {
            std::cout << "\n-----------------------------------------------------\n";
            p1Results << "\n-----------------------------------------------------\n";
            std::cout << "|                    MULTIPLY ADD                   |" << std::endl; 
            p1Results << "|                    MULTIPLY ADD                   |" << std::endl;
            std::cout << "-----------------------------------------------------\n";
            p1Results << "-----------------------------------------------------\n";
        }

        std::cout << std::left << std::setw(30) << "LOCAL_SIZE ";
        p1Results << std::left << std::setw(30) << "LOCAL_SIZE ";
        std::cout << std::left << std::setw(30) << "GLOBAL_SIZE";
        p1Results << std::left << std::setw(30) << "GLOBAL_SIZE";
        std::cout << std::left << std::setw(30) << "NUM_WORK_GROUPS";
        p1Results << std::left << std::setw(30) << "NUM_WORK_GROUPS";
        std::cout << std::left << std::setw(30) << "GigaMultsPerSecond ";
        p1Results << std::left << std::setw(30) << "GigaMultsPerSecond"; 
        std::cout << std::endl;
        p1Results << std::endl;
    }

    std::cout << std::fixed << std::setprecision(10);
    p1Results << std::fixed  << std::setprecision(10);

    // PRINT RESULTS
    std::cout << std::left << std::setw(30) << LOCAL_SIZE;
    p1Results << std::left << std::setw(30) << LOCAL_SIZE; 
    std::cout << std::left << std::setw(30) << NUM_ELEMENTS;
    p1Results << std::left << std::setw(30) << NUM_ELEMENTS;
    std::cout << std::left << std::setw(30) << NUM_WORK_GROUPS;
    p1Results << std::left << std::setw(30) << NUM_WORK_GROUPS;
    std::cout << std::left << std::setw(30) << GigaMultsPerSecond;
    p1Results << std::left << std::setw(30) << GigaMultsPerSecond;
