 ...
 * CS475
 * Program #: 7A
 * Program Name: OpenCL / OpenGL Particle System
 ******************************************************************************/
...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Chris Dubbs" };
const char *GLUITITLE   = { "User Interface Window" };

// random parameters:					
const float XMIN = 	{ -100.0 };
const float XMAX = 	{  100.0 };
const float YMIN = 	{ 550.0 };			// Repositioned to be placed between inner & outer spheres
const float YMAX = 	{  650.0 };
...
const int NUM_PARTICLES = 65536;	 //20*1024*1024;
const int LOCAL_SIZE    = 128;
const char *CL_FILE_NAME   = { "dubbsc_particles.cl" };
...
Display( )
{
	...
	if( ShowPerformance )
	{
		...
		// Track peak performance for first 1000 frames
		performance = 0.0;
		
		performance = (double)NUM_PARTICLES/ElapsedTime/1000000000.;

		//std::cout << "Frame: " << Frame << std::endl; 
		//std::cout << "Performance: " << performance << std::endl;
		//std::cout << "Peak Performance: " << peakPerformance << std::endl;

		if(performance > peakPerformance && Frame > 4)
		{
		...
		if(Frame == 1004)
		{
			...
			std::cout << "Peak Performance at 1000 frames: " << peakPerformance << std::endl; 
		}
	}
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		...
		// Sphere 1
		glColor3f( .8f, .8f, .8f);			// Outer wireframe color
		glPushMatrix( );
			glTranslatef( -0., -0., 0. );
			glutWireSphere( 1000., 20., 20. );	// sparse outer frame
		glPopMatrix( );

		// Sphere 2
		glColor3f( .8f, .8f, .8f);			// Inner wireframe color
		glPushMatrix( );
			glTranslatef( -0.f, -0.f, 0.f );
			glutWireSphere( 300, 900., 900. );	// dense inner frame
		glPopMatrix( );
	glEndList( );
...
ResetParticles( )
{
	...
	for( int i = 0; i < NUM_PARTICLES; i++ )
	{
		colors[i].r = .196078;  //Ranf( .3f, 1. );
		colors[i].g = .8;	 	//Ranf( .3f, 1. );
		colors[i].b = .196078;  //Ranf( .3f, 1. );
		colors[i].a = 1.;
	}
	glUnmapBuffer( GL_ARRAY_BUFFER );
